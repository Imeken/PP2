...
def add_player_score(connection, name, score):
    cursor = connection.cursor()
    try:
        cursor.execute("INSERT INTO snake (name, score) VALUES (%s, %s)", (name, score))
        connection.commit()
        print("Player score added successfully.")
    except psycopg2.Error as e:
        print("Error: ", e)
    finally:
        cursor.close()
...
def main():
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                add_player_score(connection, player_name, food_collected)
                running = False
            elif event.type == pygame.KEYDOWN:
